<?php 
    ...
    $fullSpace = shell_exec('df -H /"$poolName"/ | tail -n 1 | cut -d " " -f 3 | tr -d "G,T,M" ');
    $usedSpace = shell_exec('df -H /"$poolName"/ | tail -n 1 | cut -d " " -f 5 | tr -d "G,T,M" ');
    $freeSpace = shell_exec('df -H /"$poolName"/ | tail -n 1 | cut -d " " -f 7 | tr -d "G,T,M" ');
